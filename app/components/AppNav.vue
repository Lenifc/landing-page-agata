  ...
  <header
    ...
    <nav class="mx-auto flex max-w-6xl items-center justify-between px-6 py-2">
      <NuxtLink :to="ROUTES.home" class="flex items-center" aria-label="Talkateria - strona główna" @click="closeMenus">
        <img src="/talkateria-logo-color.svg" alt="Talkateria - angielski w Rumi" class="h-11 w-auto sm:h-12"
          width="188" height="56" decoding="async" />
      </NuxtLink>
      ...
      <ul class="hidden items-center gap-7 md:flex">
        ...
        <li class="relative" @mouseenter="examMenuOpen = true" @mouseleave="closeExamMenu"
          @focusout="handleExamFocusout" @keydown.esc="closeExamMenu">
          <button id="exam-menu-button" type="button"
            ...
            :class="{ '!text-foreground': examMenuActive }" aria-haspopup="true" :aria-expanded="examMenuOpen"
            aria-controls="exam-menu" :aria-label="examMenuLabel" @click="toggleExamMenu">
            <span>Egzaminy</span>
            <svg class="h-4 w-4 transition-transform duration-200" :class="{ 'rotate-180': examMenuOpen }"
              aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
              stroke-linecap="round" stroke-linejoin="round">
              <path d="m6 9 6 6 6-6" />
            </svg>
          </button>
          <div v-show="examMenuOpen" class="absolute left-1/2 top-full z-50 w-56 -translate-x-1/2 pt-3">
            <div id="exam-menu" role="menu" aria-labelledby="exam-menu-button"
              class="rounded-xl border border-border bg-card p-2 shadow-xl">
              <NuxtLink v-for="link in examLinks" :key="link.to" :to="link.to" role="menuitem"
                ...
                active-class="!text-foreground" @click="closeExamMenu">
                {{ link.label }}
              </NuxtLink>
            </div>
          </div>
      ...
      <button
        ...
        :aria-expanded="open" aria-label="Przełącz menu" @click="open = !open">
    ...
    <div v-if="open" class="border-t border-border bg-background md:hidden">
      <ul class="mx-auto flex max-w-6xl flex-col px-6 py-4">
        <li v-for="link in primaryLinks" :key="link.to">
          <NuxtLink :to="link.to" class="block py-3 text-sm font-medium text-foreground" @click="closeMenus">
            {{ link.label }}
          </NuxtLink>
        </li>
        <li class="py-2">
          ...
          <ul class="border-l border-border pl-4">
            <li v-for="link in examLinks" :key="link.to">
              <NuxtLink :to="link.to" class="block py-2.5 text-sm font-medium text-foreground" @click="closeMenus">
                {{ link.label }}
              </NuxtLink>
            </li>
          </ul>
